...
export const CompletedAgreementsTable = ({ outParams, api }) => {
  const {
    data,
    register,
    hasMore,
  ...
  const fetchData =(pageValue = page, paramsValue = params) => {
    dispatch(
      getCompletedGrants({ ...paramsValue, page_id: pageValue, limit: DEFAULT_API_RECORDS }, (results, isHasMore) => {
  ...
  const openGrantAgreementDialog = (item) => {
    openDialog(<GrantAgreementDialog grantId={item.id} data={item} hideTools />);
  }
  ...
  return (
    <Table
      {...register}
      className="completed-agreement-table h-full"
